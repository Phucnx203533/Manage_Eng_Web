...
word_template_path = base_dir / "salary.docx"
...
@app.route('/exportsalary/<string:code>')
def exportsalary(code):
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT * FROM salary WHERE salary.MaLuong = %s",(code,))
    numberteach = cursor.fetchone()
    cursor.execute("SELECT teacher.HoTen,teacher.MaGv FROM teacher WHERE teacher.MaGV = %s",(_userId,))
    data = cursor.fetchone()
    bsalary = numberteach[1]*numberteach[2]
    doc = DocxTemplate(word_template_path)
    sum = bsalary+numberteach[5]
    context={
        "day":day,
        "month":month,
        "year":year,
    ...
    doc.render(context)
    print(base_dir)
    doc.save(base_dir/f"{numberteach[6]}.docx")
